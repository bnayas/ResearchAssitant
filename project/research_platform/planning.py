...
Tier 1 — DeepPlanner
  ...
  planning loop:
    ...
    Step 0  : generate_agenda  — "what needs to be resolved?"
...
@dataclass
class ThinkingStep:
    """One planning question to resolve before committing to a plan."""
    step_id: str
    question: str
...
class ThinkingAgenda:
    ...
    *what the orchestrator needs to resolve* before acting.
    """
...
{
  "reasoning": "why these steps in this order, derived from directive + planning answers",
  "steps": [
    {
      "tool": "tool_name",
      "inputs": {"key": "value"},
...
- Apply every constraint produced during the planning phase.
...
- Do NOT assume a fixed workflow shape (e.g. article → literature → simulation).
  The steps must follow solely from the directive and the planning answers.
"""


class DeepPlanner:
    """
    Multi-step planner that resolves planning questions before acting.
    ...
    def plan(
        ...
        *,
        ...
        # Phase 0: agenda
        agenda = self._generate_agenda(directive_block, catalog_json, tool_names)
        log.info("Deep planner: agenda with %d planning steps", len(agenda.steps))
        ...
        for step in agenda.steps[: self._max_thinking_steps]:
            ...
            if result.confidence < self._confidence_warn:
                log.warning(
                    "Low confidence (%.2f) on planning step %s: %s",
        ...
        plan = self._synthesize_plan(
            directive_block, catalog_json, agenda, thinking_results, constraints, tool_names
        )
    ...
    def _generate_agenda(
        ...
        try:
            ...
            for i, item in enumerate(payload.get("steps") or []):
                ...
                context_keys = [
                    str(name).strip()
                    for name in (item.get("context_keys") or [])
                    if str(name).strip() in tool_names
                ]
...
def _keyword_score(text: str, keywords: list[str]) -> float:
    ...
    if any(" " in kw.strip() for kw in matched):
        return 0.9
    matches = len(matched)
...
def _parse_json(raw: Any) -> dict[str, Any]:
    """Parse a raw LLM response as JSON, accepting fenced or prefixed text."""
