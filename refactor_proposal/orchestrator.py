...
1. Deep planning (DeepPlanner)
   ...
   structured chain-of-thought loop:
   ...
   The full trace is emitted as a "🧠 Thinking" message so the PI can
   inspect the orchestrator's reasoning before execution begins.
...
class Orchestrator:
    ...
    def plan_deep(
        ...
        *,
        ...
        Phase 0 — generate thinking agenda ("what do I need to figure out?")
        Phase 1-N — resolve each thinking step
        Final — synthesize the ExecutionPlan from the results

        Emits a '🧠 Thinking' mailbox message with the full planning trace
        so the PI can inspect the reasoning before execution begins.
        """
        ...
        # Build a human-readable thinking trace for the mailbox
        agenda_lines = "\n".join(
            f"  {s.step_id}. {s.question}" for s in result.agenda.steps
        ) or "  (no thinking steps generated)"
        ...
        body = (
            ...
            f"Thinking agenda ({len(result.agenda.steps)} questions):\n{agenda_lines}\n\n"
            f"Resolved answers:\n{answer_lines}\n\n"
